...
# create logger
module_logger = logging.getLogger('fd_tone_notify_extension.zello')

def zello_create_token(config):
    module_logger.debug("Creating Zello Token")
...
def zello_convert(local_mp3_path):
    module_logger.debug("Converting Audio to Opus")
...
class ZelloSend:
    def __init__(self, config, audio_file):
        self.logger = logging.getLogger('fd_tone_notify_extension.zello.ZelloSend')
        self.config = config
    ...
    async def zello_stream_send_audio(self, session, ws, stream_id, opus_stream):
        ...
        while True:
            ...
            if not data:
                self.logger.info("Completed Zello Stream")
                break
...
#config.token = zello_create_token(config)
#opus_file = zello_convert("/home/icad/icad/audio_recordings/2022/08/08/Wysox_21_2022_08_08_14_03_15.mp3")
#ZelloSend(config, opus_file).zello_init_upload()